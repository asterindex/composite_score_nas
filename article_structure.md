...
5. Провести експериментальне дослідження на VisDrone2019-DET з robust нормалізацією для стабільності результатів
...
Експерименти проведено локально з автоматичним визначенням доступного прискорювача:

1. **Apple M2 Pro:**
   - Процесор: 10 cores
   - Оперативна пам'ять: 16 GB
   - Прискорювач: MPS (Metal Performance Shaders) через PyTorch
   - Час синтезу: ~15-18 хвилин (30 trials)

2. **CPU (fallback):**
   - Використовується якщо GPU/MPS недоступні
   - Час синтезу: ~45-50 хвилин (30 trials)

**Програмне забезпечення:**

- Python: 3.8+
- PyTorch: 2.0+ (з автоматичним визначенням CUDA/MPS/CPU)
- Optuna: 3.0+ (TPE sampler)
- TorchVision: 0.15+
- NumPy, Pandas, Matplotlib для аналізу
...
На відміну від більшості підходів NAS, які повідомляють тільки про фінальні архітектури, запропонована система забезпечує **повну відтворюваність** через детальне логування, збереження калібровних статистик та robust нормалізацію для стабільності на різних апаратних платформах.
...
[2026-01-24 12:34:56 UTC] 📍 Platform: MPS | Device: Apple M2 Pro
...
Для верифікації відтворюваності реалізовано валідацію через повторний запуск з однаковим SEED на різних апаратних платформах (MPS/CUDA/CPU).
...
| **Robust нормалізація** | Не застосовується | MPS/CUDA/CPU з SEED=42 |
...
## 3.5. Відтворюваність результатів

[Аналіз стабільності DSS на різних апаратних платформах з SEED=42]
